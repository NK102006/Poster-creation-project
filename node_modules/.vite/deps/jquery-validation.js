      ...
      $.extend($.validator, {
        ...
        prototype: {
          ...
          addErrorAriaDescribedBy: function(element, error, updateGroupMembers) {
            ...
            var errorID, v, group, describedBy = $(element).attr("aria-describedby");
          ...
          removeErrorAriaDescribedBy: function(element, error) {
            var describedBy = $(element).attr("aria-describedby"), describedByIds = describedBy.split(" "), errorID = error.attr("id"), ind = describedByIds.indexOf(errorID);
            if (ind > -1) {
          ...
          hideThese: function(errors) {
            for (var i = 0; errors[i]; i++) {
              var error = $(errors[i]), errorID = error.attr("id") ? this.escapeCssMeta(error.attr("id")) : void 0, element = errorID ? this.elements().filter('[aria-describedby~="' + errorID + '"]') : [];
          ...
          elements: function() {
            ...
            elements = $(this.currentForm).find(selectors.concat(this.settings.customElements).join(", ")).not(":submit, :reset, :image, :disabled").not(this.settings.ignore);
            if (formId) {
              elements = elements.add(
                $(selectors.concat(this.settings.customElements).join(", ")).filter("[form='" + validator.escapeCssMeta(formId) + "']").not(":submit, :reset, :image, :disabled").not(this.settings.ignore)
              );
            }
          ...
          showLabel: function(element, message) {
            var place, error = this.errorsFor(element), elementID = this.idOrName(element), describedBy = $(element).attr("aria-describedby");
            if (error.length) {
          ...
          findByName: function(name) {
            var formId = this.currentForm.getAttribute("id"), selector = "[name='" + this.escapeCssMeta(name) + "']", elements = $(this.currentForm).find(selector);
            if (formId) {
              elements = elements.add(
                $(selector).filter("[form='" + this.escapeCssMeta(formId) + "']")
              );
            }
            return elements;
          },
